...
for a = -11, 10 do
  for b = -11, 10 do
    ...
    if (center - Point:new(4, 0.2, 0)):length() > 0.9 then
      ...
      table.insert(objects, Sphere:new(center, 0.2, sphere_material))
    end
  end
end

make_ground()
table.insert(objects, Sphere:new(Point:new(0, 1, 0), 1.0), Dielectric:new(Dielectric.RefractiveIndex.GLASS))
table.insert(objects, Sphere:new(Point:new(-4, 1, 0), 1.0, Lambertian:new(Color:new(0.4, 0.2, 0.1))))
table.insert(objects, Sphere:new(Point:new(4, 1, 0), 1.0, Metal(Color:new(0.7, 0.6, 0.5), 0)))
